...
import type { MotifAnswers, LongevityType, PracticeType, StressType } from '../App';

interface MotifCanvasProps {
  answers: MotifAnswers;
}

// Liquid color palettes for longevity (foundation colors)
const longevityLiquidColors: Record<LongevityType, { base: string; particles: string[] }> = {
...
// Flow patterns for practice (how liquid moves)
const practiceFlowTypes = {
...
// Tension effects for stress (disturbances in the liquid)
const stressTensionTypes = {
...
export const MotifCanvas: React.FC<MotifCanvasProps> = ({ answers }) => {
  ...
  useEffect(() => {
    ...
    const initializeParticles = () => {
      ...
      if (answers.longevity) {
        const longevityData = longevityLiquidColors[answers.longevity];
        const particleCount = 200;
        
        for (let i = 0; i < particleCount; i++) {
          const colorIndex = Math.floor(Math.random() * longevityData.particles.length);
          newParticles.push({
            x: Math.random() * width,
            ...
            color: longevityData.particles[colorIndex],
      ...
      if (answers.practice) {
        const flowData = practiceFlowTypes[answers.practice];
        const particleCount = 120;
        
        for (let i = 0; i < particleCount; i++) {
          const baseColor = answers.longevity 
            ? longevityLiquidColors[answers.longevity].base 
            : '#9CA9BA';
      ...
      if (answers.stress) {
        const tensionData = stressTensionTypes[answers.stress];
    ...
    const animate = () => {
      ...
      if (answers.longevity && fillLevelRef.current > 0.01) {
        const longevityData = longevityLiquidColors[answers.longevity];
        const fillHeight = height * fillLevelRef.current;
        
        // Main liquid body
        const liquidGradient = ctx.createLinearGradient(0, height - fillHeight, 0, height);
        liquidGradient.addColorStop(0, longevityData.base + '20');
        liquidGradient.addColorStop(0.5, longevityData.base + '40');
        liquidGradient.addColorStop(1, longevityData.base + '60');
        ...
        shimmerGradient.addColorStop(0, longevityData.base + '00');
        shimmerGradient.addColorStop(0.5, longevityData.base + '50');
        shimmerGradient.addColorStop(1, longevityData.base + '20');
        ctx.fillStyle = shimmerGradient;
        ctx.fillRect(0, height - fillHeight - 30, width, 40);
      }

      // Update and draw particles with liquid behavior
      const flowData = answers.practice ? practiceFlowTypes[answers.practice] : null;
      const tensionData = answers.stress ? stressTensionTypes[answers.stress] : null;
